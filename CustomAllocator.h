...
#include <memory>
    ...
    template <class T, std::size_t growSize = 10>
    class MemoryPool
    {
        struct Block
        {
    ...
    {
#ifdef _WIN32
        CustomAllocator* copyAllocator = nullptr;
    ...
    public:
        ...
        CustomAllocator(CustomAllocator& allocator) :
            copyAllocator(&allocator)
        {
        }

        template <class U>
        CustomAllocator(const CustomAllocator<U, growSize>& other)
        {
